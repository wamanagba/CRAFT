...
## Levels, one should make sure that the Shape file is prepared as the example file
## note that, it is highly efficient for small polygon, but for a big polygon, it will
## generate the schema slowly
######################################################################################
...
# a<-Sys.time() # just to test time elapse
library(sp)
library(raster)
library(rgeos)
...
for (i in 1:3) { ## note 3 is the Levels of Shape file you have
  ...
  if(CheckFolder(source_folder,i)==1){
    ...
    for (j in 1:nrow(tmp@data)) {
      ...
      fileout <- paste0(getwd(), "/Schema/", "5m_", tname,"_", rname, ".txt")
...
cat("Schema generation Successful for all Levels.", "\n")
